...
export default function QuizSetup_oc_writing({ userType, centerCode }) {
  console.log("QuizSetup_oc_writing rendered");
  ...
  try {
    ...
    alert("✅ OC Writing exam setup saved successfully!");
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        data.detail || "Failed to save OC writing homework exam"
      );
    }

    alert("✅ OC Writing Homework exam saved successfully!");
    ...
    console.error(error);
    alert(error.message || "❌ Error saving OC writing homework exam.");
  } finally {
    setLoading(false);
  }
};
